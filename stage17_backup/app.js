...
function serializeMeshes(meshes) {
    let totalBytes = 4; // numMeshes
    for (let m of meshes) {
        totalBytes += 12; // color
        totalBytes += 4; // numPositions
        totalBytes += m.attributes.position.array.length * 4;
        totalBytes += 4; // numIndices
        totalBytes += m.index.array.length * 4;
    }
    ...
    view.setUint32(offset, meshes.length, true); offset += 4;
    for (let m of meshes) {
        let c = m.color || [0.6, 0.6, 0.6];
        view.setFloat32(offset, c[0], true); offset += 4;
        view.setFloat32(offset, c[1], true); offset += 4;
        view.setFloat32(offset, c[2], true); offset += 4;
        
        let pArr = m.attributes.position.array;
        view.setUint32(offset, pArr.length, true); offset += 4;
        new Float32Array(buffer, offset, pArr.length).set(pArr);
        offset += pArr.length * 4;
        
        let iArr = m.index.array;
        view.setUint32(offset, iArr.length, true); offset += 4;
        new Uint32Array(buffer, offset, iArr.length).set(iArr);
        offset += iArr.length * 4;
    }
    return buffer;
}

function deserializeMeshes(buffer) {
    let view = new DataView(buffer);
    let offset = 0;
    let numMeshes = view.getUint32(offset, true); offset += 4;
    let meshes = [];
    
    for (let i = 0; i < numMeshes; i++) {
        let color = [view.getFloat32(offset, true), view.getFloat32(offset+4, true), view.getFloat32(offset+8, true)];
        offset += 12;
        let pLen = view.getUint32(offset, true); offset += 4;
        let pArr = new Float32Array(buffer.slice(offset, offset + pLen*4));
        offset += pLen * 4;
        let iLen = view.getUint32(offset, true); offset += 4;
        let iArr = new Uint32Array(buffer.slice(offset, offset + iLen*4));
        offset += iLen * 4;
        meshes.push({ color: color, attributes: { position: { array: pArr } }, index: { array: iArr } });
    }
    return meshes;
}

async function saveFileDB(key, bufferOrData) {
    let dataToSave = bufferOrData;
    if (key.endsWith('_step') && window.pako) {
        try {
            if (bufferOrData.isOptimizedMesh) {
                const binBuffer = serializeMeshes(bufferOrData.optimizedMeshes);
                const compressed = pako.deflate(new Uint8Array(binBuffer));
                dataToSave = { isPakoCompressed: true, isOptimizedMesh: true, data: compressed };
                console.log(`Mesh serialized & compressed: ${(binBuffer.byteLength / 1024).toFixed(1)} KB -> ${(compressed.byteLength / 1024).toFixed(1)} KB`);
            } else if (bufferOrData instanceof ArrayBuffer) {
                const uint8 = new Uint8Array(bufferOrData);
                const compressed = pako.deflate(uint8);
                dataToSave = { isPakoCompressed: true, data: compressed };
            }
        } catch (e) {
    ...
    return new Promise(r => {
        ...
        tx.objectStore('files').put(dataToSave, key);
        tx.oncomplete = () => r();
    });
}
...
async function loadFileDB(key) {
    ...
    return new Promise(r => {
        ...
        req.onsuccess = () => {
            ...
            if (result && result.isPakoCompressed && window.pako) {
                try {
                    const decompressed = pako.inflate(result.data);
                    if (result.isOptimizedMesh) {
                        result = {
                            isOptimizedMesh: true,
                            optimizedMeshes: deserializeMeshes(decompressed.buffer.slice(decompressed.byteOffset, decompressed.byteOffset + decompressed.byteLength))
                        };
                    } else {
    ...
    if (ev.dataTransfer.files && ev.dataTransfer.files.length > 0) {
        ...
        if (type === 'excel') {
            ...
            file.arrayBuffer().then(async buf => {
                currentOrderData.stepName = file.name;
                let n = file.name.toLowerCase();
                
                if (n.endsWith('.x_t') || n.endsWith('.x_b') || n.endsWith('.stl')) {
                    currentOrderData.stepBuffer = buf;
                    document.getElementById('btn3D')?.click();
                } else {
                    const statusEl = document.getElementById('dropSTEP')?.querySelector('.status');
                    if (statusEl) statusEl.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Dönüştürülüyor...';
                    
                    if (!window.myOCCT) {
                        window.myOCCT = await occtimportjs({
                    ...
                    setTimeout(() => {
                        try {
                            let result = window.myOCCT.ReadStepFile(new Uint8Array(buf), { linearDeflection: 0.3, angularDeflection: 0.3 });
                            if (result && result.meshes) {
                                currentOrderData.optimizedMeshes = result.meshes;
                                currentOrderData.isOptimizedMesh = true;
                                currentOrderData.stepBuffer = null;
                            } else {
                                currentOrderData.stepBuffer = buf;
                            }
                        } catch(e) {
...
async function init3DViewer() {
    if (!currentOrderData.stepBuffer && !currentOrderData.isOptimizedMesh) {
    ...
    // Yüklenen dosya .x_t veya .x_b mi kontrolü
    if (currentOrderData.stepName) {
        let n = currentOrderData.stepName.toLowerCase();
        if (n.endsWith('.x_t') || n.endsWith('.x_b')) {
            container.innerHTML = `
                <div class="empty-state" style="display:flex; flex-direction:column; align-items:center; color: var(--text);">
                    <i class="fas fa-file-shield" style="font-size:48px; color:var(--primary); margin-bottom:15px; opacity:0.8;"></i>
                    <h3 style="margin-bottom:10px;">Görüntüleme Desteklenmiyor</h3>
                    <p style="max-width:80%; text-align:center; opacity:0.8; line-height:1.5;">
                        <strong>${currentOrderData.stepName}</strong> başarıyla siparişe eklendi.
                        <br><br>
                        Üçüncü parti (.x_t, .x_b) Parasolid kalıpları, performans ve lisans sebepleriyle tarayıcının yerleşik 3D motoru üzerinden işlenememektedir. Yerel CAD programınız ile açmak için aşağıdaki butonu kullanabilirsiniz.
                    </p>
                    <button onclick="downloadParasolidFile()" class="btn" style="margin-top:20px; background:var(--primary); color:white; border:none; box-shadow:0 0 10px rgba(0, 204, 153, 0.3);">
                        <i class="fas fa-download"></i> Orijinal Dosyayı İndir
                    </button>
                </div>
            `;
            
            // Eğer daha önceden tanımlanmadıysa global indirme fonksiyonunu tanımla
            if (!window.downloadParasolidFile) {
                window.downloadParasolidFile = function() {
                    if(!currentOrderData.stepBuffer || !currentOrderData.stepName) return;
                    const blob = new Blob([currentOrderData.stepBuffer], { type: 'application/octet-stream' });
                    const url = URL.createObjectURL(blob);
                    const a = document.createElement('a');
                    a.href = url;
                    a.download = currentOrderData.stepName;
            ...
            try {
                ...
                const geometry = loader.parse(currentOrderData.stepBuffer);
                ...
                mesh.position.sub(center.multiplyScalar(100 / maxDim));
                ...
                console.error("STL Render Error:", err);
                container.innerHTML = `<div class="empty-state">STL okunamadı: ${err.message}</div>`;
            }
            return;
        }
    }
    ...
    setTimeout(async () => {
        try {
            ...
            if (currentOrderData.isOptimizedMesh) {
                meshes = currentOrderData.optimizedMeshes;
            } else {
                if (!myOCCT) {
                ...
                let result = myOCCT.ReadStepFile(new Uint8Array(currentOrderData.stepBuffer), params);
                meshes = result.meshes;
                
                // Cache it so we don't need the buffer anymore
                currentOrderData.optimizedMeshes = meshes;
                currentOrderData.isOptimizedMesh = true;
                currentOrderData.stepBuffer = null;
            }
...
function loadOrderDetails() {
    ...
    // Hydrate Production Hours
    if (order.designHours !== undefined) setV('h-design', order.designHours);
    if (order.weldingHours !== undefined) setV('h-welding', order.weldingHours);
    if (order.paintHours !== undefined) setV('h-paint', order.paintHours);
    if (order.montajHours !== undefined) setV('h-assembly', order.montajHours);
    ...
    if (order.stepName) {
        currentOrderData.stepName = order.stepName;
    }
...
document.getElementById('saveOrderBtn')?.addEventListener('click', () => {
    ...
    let order = {
        ...
        stepName: currentOrderData.stepName || '',
        progress: actualProgress,
        completedStages: completedStagesArray,
        designHours: parseFloat(document.getElementById('h-design')?.value) || 0,
        weldingHours: parseFloat(document.getElementById('h-welding')?.value) || 0,
        paintHours: parseFloat(document.getElementById('h-paint')?.value) || 0,
    ...
    if (order.progress < 100 && order.deliveryDate) {
        ...
        if(!isNaN(targetDate)) {
            ...
            while(!isAvailable) {
                let baseCap = weekCaps[currentWeekPointer] !== undefined ? parseInt(weekCaps[currentWeekPointer]) : 25;
                
                // --- NEW LOGIC: Subtract Used Hours from Personnel Logs ---
                let lostCapacity = 0;
                let personnelLogs = JSON.parse(localStorage.getItem('hg_personnel_logs') || '[]');
                personnelLogs.forEach(log => {
                    let logDate = new Date(log.date);
                    if (!isNaN(logDate.getTime())) {
                        let logWeek = getISOWeek(logDate);
                        if (logWeek === currentWeekPointer) {
                            lostCapacity += (parseFloat(log.hoursUsed) || 0);
                        }
                    }
                });
                
                let maxCap = baseCap - lostCapacity;
                if (maxCap < 0) maxCap = 0; // Prevent negative capacity
...
function renderDashboard() {
    ...
    grid.innerHTML = '';
    orders.forEach(o => {
        let targetIndex = 0; // 0: Tasarım, 1: Kaynak, 2: Boya, 3: Montaj
        if (o.completedStages && o.completedStages.length > 0) {
            ...
            if(o.progress > 25) targetIndex = 1;
            if(o.progress > 50) targetIndex = 2;
            if(o.progress >= 100) targetIndex = 3;
        }

        let stageName = 'Tasarım';
        let pColor = 'var(--c-red)';
        
        if (targetIndex === 0) { stageName = 'Tasarım'; pColor = 'var(--c-red)'; }
        if (targetIndex === 1) { stageName = 'Kaynak'; pColor = 'var(--c-yellow)'; }
        if (targetIndex === 2) { stageName = 'Boya'; pColor = 'var(--c-orange)'; }
        if (targetIndex === 3) { stageName = 'Montaj'; pColor = 'var(--c-green)'; }
    ...
    // Call our new sidebar reports function with ALL orders (unfiltered)
    renderSidebarReports(JSON.parse(localStorage.getItem('hg_orders') || '[]'));
}
...
function renderSidebarReports(orders) {
    if (!document.getElementById('sidebarReports')) return;
    ...
    let stats = {
        daily: { in: 0, out: 0, hrs: 0 },
        weekly: { in: 0, out: 0, hrs: 0 },
        monthly: { in: 0, out: 0, hrs: 0 },
        yearly: { in: 0, out: 0, hrs: 0 }
    };

    orders.forEach(o => {
        let orderDate = o.orderDate ? new Date(o.orderDate) : null;
        let isIncoming = orderDate && !isNaN(orderDate.getTime());
        
        let deliveryDate = o.deliveryDate ? new Date(o.deliveryDate) : null;
        let isOutgoing = o.progress === 100 && deliveryDate && !isNaN(deliveryDate.getTime());
        
        let qty = parseInt(o.qty) || 1;
        let totalHrs = (parseFloat(o.designHours) || 0) + 
                       (parseFloat(o.weldingHours) || 0) + 
                       (parseFloat(o.paintHours) || 0) + 
                       (parseFloat(o.montajHours) || 0);

        // DAILY
        if (isIncoming && orderDate.toISOString().split('T')[0] === todayStr) {
            stats.daily.in += qty;
        }
        if (isOutgoing && deliveryDate.toISOString().split('T')[0] === todayStr) {
            stats.daily.out += qty;
            stats.daily.hrs += totalHrs;
        }

        // WEEKLY
        if (isIncoming && orderDate.getFullYear() === currentYear && getISOWeek(orderDate) === currentWeek) {
            stats.weekly.in += qty;
        }
        if (isOutgoing && deliveryDate.getFullYear() === currentYear && getISOWeek(deliveryDate) === currentWeek) {
            stats.weekly.out += qty;
            stats.weekly.hrs += totalHrs;
        }

        // MONTHLY
        if (isIncoming && orderDate.getFullYear() === currentYear && orderDate.getMonth() === currentMonth) {
            stats.monthly.in += qty;
        }
        if (isOutgoing && deliveryDate.getFullYear() === currentYear && deliveryDate.getMonth() === currentMonth) {
            stats.monthly.out += qty;
            stats.monthly.hrs += totalHrs;
        }

        // YEARLY
        if (isIncoming && orderDate.getFullYear() === currentYear) {
            stats.yearly.in += qty;
        }
        if (isOutgoing && deliveryDate.getFullYear() === currentYear) {
            stats.yearly.out += qty;
            stats.yearly.hrs += totalHrs;
        }
    });

    const setT = (id, val) => { if(document.getElementById(id)) document.getElementById(id).innerText = val; };
    
    setT('rep-daily-in', stats.daily.in);
    setT('rep-daily-out', stats.daily.out);
    setT('rep-daily-hrs', stats.daily.hrs);

    setT('rep-weekly-in', stats.weekly.in);
    setT('rep-weekly-out', stats.weekly.out);
    setT('rep-weekly-hrs', stats.weekly.hrs);

    setT('rep-monthly-in', stats.monthly.in);
    setT('rep-monthly-out', stats.monthly.out);
    setT('rep-monthly-hrs', stats.monthly.hrs);

    setT('rep-yearly-in', stats.yearly.in);
