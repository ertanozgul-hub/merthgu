...
if (isDashboard) {
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (weeksContainer) {
            ...
            for(let i=0; i<5; i++) {
                let w = currentWeek + i;
                html += `<label style="font-size:12px; margin-bottom:2px; display:flex; align-items:center; gap:4px; opacity:0; transform:translateX(-10px); animation:slideRight 0.3s forwards ${i*0.05}s"><input type="checkbox" class="filter-week" value="${w}"> -${w}. Hafta</label>`;
            }
