...
function parseExcel(file) {
    ...
    reader.onload = function(e) {
        ...
        if (orderNoText && document.getElementById('orderLabel')) {
            ...
            document.getElementById('orderLabel').innerText = `Sipariş No: ${cleanOrderNo}`;
        }
    };
...
function setupCounter() {
    document.getElementById('qtyMinus').onclick = () => {
        ...
        if(v > 1) document.getElementById('orderQty').value = v - 1;
    }
...
function generateLabelNo() {
    ...
    // Fetch last number from localSotage (mocking)
    let lastNo = parseInt(localStorage.getItem('lastOrderNo')) || 33;
    let newNo = lastNo + 1;
    document.getElementById('orderLabel').innerText = `Sipariş No (${year}-${newNo})`;
}
